...
from helpers import (
    ...
    get_signatures,
    post_process_data,
    get_sign_ratio_from_signatures_array,
    get_rewards_current,
    get_block_time,
    get_total_supply,
...
VALIDATOR_STAKES = []
assert len(VALIDATOR_ADDRESS) == len(
    VALIDATOR_NAME
), "VALIDATOR_ADDRESS and VALIDATOR_NAME must have the same length"
...
N_ACTIVE_VALIDATORS = int(
    requests.get(RPC_URL + "/validatorsets/latest", headers=headers).json()["result"][
        "total"
    ]
)
...
for i in range(len(VALIDATOR_ADDRESS)):
    ...
    CONST_ATTRIBUTES[f"val_{i}_commission_pct"] = get_validator_commission(
        VALIDATOR_ADDRESS[i]
    )
...
def MyThread5(res, key, _block_signatures):
    t = time.time()
    res[key] = get_sign_ratio_from_signatures_array(
        _block_signatures, N_ACTIVE_VALIDATORS
    )
...
def get_all_block_data(LATEST_BLOCK, _block_signatures, _timestamp, _proposer_addr):
    ...
    all_threads = [
        ...
        threading.Thread(
            target=MyThread5, args=[result, "sign_ratio", _block_signatures]
        ),
    ...
    for val_ind, validator in enumerate(VALIDATOR_ADDRESS):
        all_threads.append(
            threading.Thread(target=MyThread10, args=[result, validator, val_ind])
        )
    ...
    dir_path = dir_path = (
        ...
        + f"/single_validators/{datafile_name}.csv"
    )
    print(
        bcolors.OKGREEN, "result for block ", LATEST_BLOCK, " : ", bcolors.ENDC, result
    )
    try:
...
while got_blocks < N_BLOCKS_TO_GET:
    ...
    new_block_thread = threading.Thread(
        target=get_all_block_data,
        args=[new_block, block_signatures, time_stamp, proposer_addr],
    )
...
dir_path = (
    os.path.dirname(os.path.realpath(__file__))
    + f"/single_validators/{datafile_name}-processed.csv"
)
