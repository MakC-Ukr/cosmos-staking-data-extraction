...
def get_inflation():
    response = requests.get(
        RPC_URL + "/cosmos/mint/v1beta1/inflation", headers=headers
    ).json()
...
def get_timestamp(block_number=-1):
    if block_number == -1:
        response = requests.get(
            RPC_URL + "/cosmos/base/tendermint/v1beta1/blocks/latest", headers=headers
        ).json()
    else:
        response = requests.get(
            RPC_URL + "/cosmos/base/tendermint/v1beta1/blocks/" + str(block_number),
            headers=headers,
        ).json()
...
def get_supply_bonded_ratio():
    ...
    bond_ratio = requests.get(
        "https://api.cosmoscan.net/bonded-ratio/agg", params=params, headers=headers
    ).json()[-1]["value"]
    return bond_ratio
...
def get_n_active_validators():
    validator_set = requests.get(
        RPC_URL_3 + "/validatorsets/latest", headers=headers
    ).json()["result"]["total"]
    return int(validator_set)


def get_validator_stake(validator_addr):
    stake_val = requests.get(
        COSMOSCAN_API + "/validator/" + validator_addr, headers=headers
    ).json()["power"]
    return stake_val
...
def get_precommit_ratio(num_signatures):
    response = len(
        requests.get(
            RPC_URL + "/blocks/" + str(num_signatures), headers=headers
        ).json()["block"]["last_commit"]["signatures"]
    )
    return response
...
def get_chain_distribution_parameters():
    response = requests.get(
        RPC_URL + "/distribution/parameters", headers=headers
    ).json()["result"]
    result_dict = {}
...
def get_rewards_current(validator_addr):
    ...
    response = requests.get(
        RPC_URL
        + f"/cosmos/distribution/v1beta1/validators/{validator_addr}/commission",
        headers=headers,
    ).json()
    result_dict = {}
...
def get_total_supply():
    response = requests.get(
        RPC_URL + "/cosmos/bank/v1beta1/supply/uatom", headers=headers
    ).json()["amount"]["amount"]
    return response


def get_block_time(bl_num):
    response = requests.get(RPC_URL + "/blocks/" + str(bl_num), headers=headers).json()[
        "block"
    ]["header"]["time"]
    return response
...
def get_validator_commission(validator_addr):
    ...
    """
    response = requests.get(
        RPC_URL + f"/cosmos/staking/v1beta1/validators/{validator_addr}",
        headers=headers,
    ).json()["validator"]["commission"]["commission_rates"]["rate"]
        ...
        for log in logs:
            ...
            for event in events:
                if (
                    ...
                    and event["attributes"][1]["value"] == val_ad
                ):
                    amt_withdrawn = event["attributes"][0]["value"]
                    amt_withdrawn = float(
                        amt_withdrawn[:-5]
                    )  # assuming the amount is in the form of "1234uatom"
                    was_withdrawn = True
...
def post_process_data(df, N_validators):
    ...
    for ind, row in df.iterrows():
        if ind == 0:
            row[
                "block_len"
            ] = DEFAULT_BLOCK_TIME  # for the first row the block length can be set to 6.5 s
            ...
            row["block_len"] = (
                float(time_to_unix(row["timestamp"]) - prev_timestamp) / 1000.0
            )
        ...
        for i in range(N_validators):
            ...
            rew_diffs.append(
                df.iloc[ind + 1][f"val_{i}_com_amt"] - row[f"val_{i}_com_amt"]
            )
        ...
        for i in range(diff_from_next_row):
            ...
            for i in range(N_validators):
                new_row[f"val_{i}_com_amt"] = row_rewards[i] + rew_diffs[i] / (
                    diff_from_next_row + 1
                ) * (i + 1)
...
params = {
    "message.sender": "cosmos1tdghn58d6szwh5e4dyal5cyz04vk8um3wycye0",
    "page": "1",
    "limit": "10",
    "tx.minheight": "0",
    "tx.maxheight": "13173861",
}

t = time.time()
response = requests.get(RPC_URL + "/txs", params=params, headers=headers).json()
json.dump(response, open("txs.json", "w"))
print(response)
